...
\textbf{Stack:}         & React 19, Vite, Firebase, CSS personalizado \\
\textbf{Accesibilidad:} & WCAG 2.2 Nivel AA (objetivo), HTML5 semántico + WAI-ARIA selectivo \\
\textbf{Módulos:}       & 9 secciones + panel de administración \\
\textbf{Repositorio:}   & \href{https://github.com/zeuspyEC/VitaPrevent}{github.com/zeuspyEC/VitaPrevent} \\
\textbf{Despliegue:}    & Firebase Hosting --- CI/CD con GitHub Actions \\
\end{tabular}
\end{infobox}
...
\toprule
\rowcolor{royalblue!12}
\textbf{Módulo} & \textbf{Contenido principal} & \textbf{Fuente de datos} & \textbf{Acceso} \\
\midrule
Inicio          & Hero, estadísticas, módulos, CTA & Firestore + estático & Público \\
Nutrición       & Artículos, calculadora IMC & Firestore & Público \\
Actividad Física & Rutinas, videos, niveles & Firestore & Público \\
Salud Mental    & Artículos, podcasts, mindfulness & Firestore & Público \\
Prevención      & Enfermedades, vacunación, riesgos & Firestore & Público \\
Biblioteca      & Infografías, PDFs, guías & Firestore + Storage & Público \\
Noticias        & Feed dinámico paginado & Firestore & Público \\
Contacto        & Formulario accesible & Firestore (write) & Público \\
Nosotros        & Equipo, misión, valores & Estático & Público \\
Panel Admin     & CRUD completo de contenidos & Firestore & Privado (Auth) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item Completar los módulos de Nutrición, Actividad Física, Salud Mental y Prevención con contenido dinámico real desde Firestore.
  \item Implementar el panel de administración con Firebase Authentication.
  \item Realizar pruebas de usuario con personas con discapacidades reales (NVDA, VoiceOver).
  \item Obtener la certificación formal de conformidad WCAG 2.2 AA mediante auditoría externa.
  \item Añadir soporte para modo oscuro completo con respeto a \texttt{prefers-color-scheme}.
  \item Implementar internacionalización (i18n) para acceso en lenguas indígenas del Ecuador.
  \item Agregar PWA completa con soporte offline para contenidos guardados.
\end{itemize}
