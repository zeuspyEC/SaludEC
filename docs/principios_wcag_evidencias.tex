% ============================================================
%  PRINCIPIOS WCAG 2.2 Y EVIDENCIAS --- SaludEC
...
\usepackage[colorlinks=true, linkcolor=royalblue, urlcolor=royalblue, unicode=true,
  pdftitle={SaludEC - Principios WCAG y Evidencias POUR},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{graymid}\textit{SaludEC --- Principios WCAG 2.2 y Evidencias POUR}}
...
{\Huge\bfseries\color{navydeep} SaludEC}\\[0.5cm]
...
\begin{tabular}{ll}
  ...
  \textbf{Repositorio:} & \texttt{github.com/zeuspyEC/SaludEC} \\
...
SaludEC adopta el enfoque \textit{accessibility-first}: cada componente se diseña desde el principio para cumplir POUR, sin añadir accesibilidad como capa posterior.
...
\textbf{Cómo se cumple en SaludEC:}
...
\begin{itemize}
  ...
  \item \textbf{Imágenes de artículos:} El campo \texttt{imagen.alt} es obligatorio en Firestore. El componente \texttt{ArticleCard} lo consume directamente. Cada artículo tiene imagen única dentro de su sección --- no existe ningún par de artículos en la misma página que compartan la misma fotografía, evitando que un lector de pantalla anuncie el mismo \texttt{alt} dos veces para contenidos distintos.
  \item \textbf{Alt descriptivo del tema:} El texto alternativo se genera automáticamente describiendo el tema específico del artículo (p.~ej.\ ``Médico midiendo presión arterial --- exámenes preventivos esenciales'' es distinto de ``Manos con estetoscopio --- consulta preventiva''), no un texto genérico común para todos.
...
Además de la estructura semántica de la página, el \textbf{cuerpo del contenido} de artículos y noticias se renderiza preservando su jerarquía HTML. El contenido almacenado en Firestore proviene del editor WYSIWYG del panel administrativo (formato HTML) o de texto Markdown. El componente \texttt{ArticleDetail} detecta automáticamente el formato y renderiza HTML con \texttt{dangerouslySetInnerHTML} o Markdown con \texttt{ReactMarkdown}, garantizando que etiquetas como \texttt{<h2>}, \texttt{<p>} y \texttt{<strong>} se transmitan correctamente al DOM --- no como texto plano --- para que los lectores de pantalla interpreten la jerarquía de encabezados y el énfasis semántico del contenido.
...
\textbf{Verificación de ruta Tab en SaludEC:}
...
\begin{table}[H]
\centering
\caption{Cumplimiento POUR global de SaludEC}
...
\midrule
Perceptible & \textcolor{successgreen}{\bfseries 96\%} & 1.1.1 alt text único, 1.2.2 vídeo con subtítulos VTT, 1.3.1 semántica HTML, 1.4.3 contraste corregido (Sprint 2), 1.4.10 responsive 320px. Parcial: 1.2.3 audiodescripción \\
...
Robusto & \textcolor{warningyellow}{\bfseries 95\%} & 4.1.1 HTML válido, 4.1.3 aria-live. Parcial: 4.1.2 atributos ARIA prohibidos en 2 elementos (Lighthouse, deuda técnica) \\
\midrule
\textbf{Global WCAG 2.2 AA} & \textcolor{warningyellow}{\bfseries 92\%} & Lighthouse Accessibility 92/100 (13/07/2026); 4 criterios con cumplimiento parcial; contraste 1.4.3 corregido en Sprint 2 \\
\bottomrule
\end{tabular}
\end{table}

\subsection*{Evidencias de herramientas de evaluación (13/07/2026)}

\begin{table}[H]
\centering
\caption{Resultados reales de herramientas de accesibilidad}
\begin{tabular}{@{}>{\bfseries}p{4.5cm} >{\centering\arraybackslash}p{3.5cm} p{6cm}@{}}
\toprule
\rowcolor{royalblue!12}
\textbf{Herramienta} & \textbf{Resultado} & \textbf{Detalle} \\
\midrule
axe DevTools v4.9 & \textcolor{successgreen}{0 violaciones} & 0 críticas, 0 graves, 0 moderadas en todas las páginas \\
Lighthouse 13.4.0 & \textcolor{warningyellow}{92/100} & 1 issue ARIA prohibido (4.1.2, deuda técnica) \\
WAVE v3.2.7 & \textcolor{successgreen}{0 errores} & 2 alertas SPA (falsos positivos documentados) \\
Pa11y 9.1.1 (CLI) & \textcolor{successgreen}{0 errores reales} & 7 issues de contraste corregidos; 6 FP documentados \\
NVDA 2024.1 + Chrome & \textcolor{successgreen}{Accesible} & Landmarks, jerarquía H1→H3, aria-live correctos \\
Teclado Tab/Esc & \textcolor{successgreen}{Accesible} & Sin trampas de foco; todas las funciones operables \\
PageSpeed (rendimiento) & \textcolor{warningyellow}{Móvil 66/100} & CLS 0.326, FCP 5.0s (deuda técnica, no WCAG) \\
\bottomrule
\end{tabular}
\end{table}

Pa11y se ejecutó desde CLI con Chromium headless el 13/07/2026. Antes de las correcciones de Sprint 2, reportó 7 issues de contraste reales en \texttt{/nosotros}; todos fueron corregidos ese mismo día. La ejecución final en las tres rutas principales produjo \texttt{No issues found!} en las tres. Los 6 falsos positivos restantes se deben a que pa11y evalúa el gradiente CSS del \texttt{SectionHero} como fondo plano (ratio calculado 1:1; ratio real $\approx$17:1) y a contenido de fallback \texttt{<video>} que no se renderiza en navegadores modernos.

\begin{okbox}[Declaración resumida de conformidad]
SaludEC cumple sustancialmente con los cuatro principios POUR de WCAG 2.2 Nivel AA. Resultados de herramientas al 13/07/2026: axe DevTools 0 violaciones, Lighthouse 92/100, WAVE 0 errores, Pa11y 9.1.1 sin errores reales en \texttt{/}, \texttt{/contacto} y \texttt{/accesibilidad}. En Sprint 2 (cierre 13/07/2026) se corrigieron 7 problemas de contraste 1.4.3 detectados por Pa11y y se añadió vídeo accesible con subtítulos VTT (1.2.2). Permanecen cuatro criterios con cumplimiento parcial documentado: 2.5.7 (cubo sin \texttt{aria-label} explícito), 3.2.6 (ayuda no contextual por módulo), 4.1.2 (ARIA prohibido en 2 elementos --- Lighthouse, deuda técnica), y 1.2.3 (sin audiodescripción del vídeo). El sitio \textbf{no presenta barreras que impidan} el acceso a la información de servicios públicos de salud para usuarios con discapacidades visuales, motrices, auditivas o cognitivas.
\end{okbox}
