% ============================================================
%  DOCUMENTO DE ARQUITECTURA --- SaludEC
...
\usepackage[colorlinks=true, linkcolor=royalblue, urlcolor=royalblue, unicode=true,
  pdftitle={SaludEC - Arquitectura y Analisis de Brechas},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{graymid}\textit{SaludEC --- Arquitectura y Análisis de Brechas}}
...
{\Huge\bfseries\color{navydeep} SaludEC}\\[0.4cm]
...
SaludEC sigue un patrón \textbf{SPA (Single Page Application)} con las siguientes capas:
...
src/
  ...
  pages/            # Home, Nutricion, ActividadFisica, SaludMental,
                    # Prevencion, Noticias, Contacto, Nosotros,
                    # Accesibilidad, Admin
...
\midrule
\texttt{articulos} & \texttt{titulo, slug, resumen, contenido (HTML desde editor WYSIWYG o Markdown), modulo, categoria, etiquetas, imagen.url, imagen.alt, autor, creadoEn, actualizadoEn, publicado} \\
\texttt{recursos} & \texttt{titulo, tipo, url, descripcion, categoria, etiquetas, thumbnail.url, thumbnail.alt, estado} \\
\texttt{noticias} & \texttt{titulo, slug, resumen, contenido (HTML/Markdown), imagen.url, imagen.alt, creadoEn, categoria, publicado, fuenteUrl} \\
...
\begin{table}[H]
\centering
\caption{Mapa completo de rutas --- SaludEC}
...
/biblioteca & Redirect & Redirige a Inicio (sección eliminada en Sprint 2) \\
...
ArticleDetail & Renderizado HTML/Markdown adaptivo & Detecta formato del contenido: HTML con \texttt{dangerouslySetInnerHTML} (admin WYSIWYG), Markdown con \texttt{ReactMarkdown} — preserva jerarquía semántica \\
...
Multimedia con transcripción/alt & \cmark & Vídeo oficial MSP/OPS en Nosotros con \texttt{<track kind="captions">} VTT español (WCAG 1.2.2). Audiodescripción: parcial (1.2.3). \\
...
\begin{warnbox}[Deudas técnicas documentadas --- Sprint 2 cerrado (13/07/2026)]
Las siguientes brechas permanecen como deuda técnica tras el cierre del Sprint 2:
\begin{enumerate}
  \item \textbf{WCAG 2.5.7 (AA) --- Movimientos de arrastre:} El cubo 3D usa arrastre. Alternativa funcional: rotación automática + Navbar. Pendiente: \texttt{aria-label} explícito en el contenedor del cubo.
  \item \textbf{WCAG 3.2.6 (A, WCAG 2.2) --- Ayuda consistente:} Formulario de contacto en footer (enlace consistente). Falta ayuda contextual visible dentro de cada módulo.
  \item \textbf{WCAG 4.1.2 --- ARIA prohibido:} Lighthouse detecta 2 elementos con atributos ARIA prohibidos para su rol. Funcionalidad no afectada; corrección pendiente.
  \item \textbf{WCAG 1.2.3 --- Audiodescripción:} El vídeo de Nosotros tiene subtítulos VTT (1.2.2 cumplido) pero no audiodescripción completa para usuarios ciegos.
  \item \textbf{CLS 0.326 en escritorio:} Layout shift por carga tardía de fuentes y animación shimmer no compuesta. No corregido en este sprint.
\end{enumerate}
\end{warnbox}
...
\begin{successbox}[Brechas resueltas en Sprint 2 (cierre 13/07/2026)]
\begin{itemize}
  \item \textbf{Contraste WCAG 1.4.3:} \cmark~Corregido en footer (7.4:1), estadísticas Home (10:1) y declaración de accesibilidad (5.36:1).
  \item \textbf{Vídeo accesible:} \cmark~Vídeo MSP/OPS en Nosotros con subtítulos VTT español (\texttt{<track kind="captions" default>}, WCAG 1.2.2).
  \item \textbf{Biblioteca eliminada:} \cmark~Sección eliminada; \texttt{/biblioteca} redirige a Inicio; referencias limpias en Navbar, Footer, Breadcrumb, Home y Admin.
  \item \textbf{Navegación:} \cmark~Scroll al inicio en PUSH/REPLACE (no en POP); logo/eslogan clickables; foco movido al \texttt{<main>} en cada navegación (\texttt{PageWrapper} con \texttt{appHasHydrated} módulo-nivel).
  \item \textbf{Formulario de contacto:} \cmark~Validación blur por campo sin disrumpir flujo de foco (ref \texttt{focusSummaryAfterSubmit}).
  \item \textbf{Declaración de accesibilidad:} \cmark~Honesta: Lighthouse 92/100 real, 4 criterios parciales documentados, 5 correcciones pendientes listadas.
  \item \textbf{Link directo a videos:} \cmark~Footer ``Recursos'' incluye ``Videos de salud'' → \texttt{/nosotros\#video-title}.
\end{itemize}
\end{successbox}

\begin{warnbox}[Issues activos (PageSpeed Insights 13/07/2026)]
\begin{enumerate}
  \item \textbf{ARIA prohibido (4.1.2):} Lighthouse detecta 2 elementos con atributos ARIA prohibidos. Deuda técnica documentada en declaración de accesibilidad.
  \item \textbf{CLS 0.326 en escritorio:} Layout shift por carga tardía de fuentes y animación shimmer. Deuda técnica.
  \item \textbf{Rendimiento móvil 66/100:} LCP 5,6 s. Causas: render-blocking CSS, imágenes \texttt{w=800}, JS no utilizado ($\sim$92 KiB).
\end{enumerate}
\end{warnbox}
...
6.2 Contenido & \textcolor{successgreen}{\bfseries 95\%} & Vídeo MSP/OPS con subtítulos VTT (WCAG 1.2.2). Parcial: audiodescripción (1.2.3) \\
...
7.1 WCAG 2.2 & \textcolor{warningyellow}{\bfseries 92\%} & Lighthouse 92/100 (13/07/2026); 4 criterios parciales: 1.2.3, 2.5.7, 3.2.6, 4.1.2; contraste 1.4.3 corregido \\
7.2 WAI-ARIA & \textcolor{warningyellow}{\bfseries 95\%} & 2 elementos con atributos ARIA prohibidos (Lighthouse, deuda técnica documentada) \\
