...
\begin{successbox}[Brecha resuelta en Sprint 2 (al 02/07/2026)]
\begin{itemize}
  \item \textbf{Panel Admin completo:} \cmark~Implementado. CRUD completo de artículos, noticias, recursos y mensajes. Incluye servicio de limpieza (\texttt{cleanup.service.js}) que asigna imágenes verificadas y únicas por sección a todos los documentos de Firestore automáticamente.
  \item \textbf{Imágenes únicas por sección:} \cmark~Implementado. Cada artículo recibe una fotografía visualmente distinta dentro de su módulo; sub-routing diferencia artículos del mismo tema con títulos distintos.
  \item \textbf{Renderizado HTML semántico:} \cmark~Implementado. \texttt{ArticleDetail} y \texttt{NoticiasDetalle} detectan el formato del contenido (HTML vs Markdown) y lo renderizan preservando la jerarquía semántica.
\end{itemize}
\end{successbox}
...
7.1 WCAG 2.2 & \textcolor{warningyellow}{\bfseries 96\%} & 2 criterios parciales (2.5.7, 3.2.6); 5 correcciones adicionales aplicadas en Sprint 2 \\
