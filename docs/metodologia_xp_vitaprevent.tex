% ============================================================
%  DOCUMENTO DE PLANIFICACIÓN XP --- SaludEC
...
\usepackage[
  ...
  pdftitle={SaludEC - Metodología XP - Planificación de Iteraciones},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{graymid}\textit{SaludEC --- Metodología XP --- ISWD732 GR3SW}}
...
El presente documento describe la planificación, ejecución y gestión del proyecto SaludEC empleando la metodología \textbf{Extreme Programming (XP)}, seleccionada por su adaptabilidad a equipos pequeños, su énfasis en la calidad del código y su capacidad para incorporar cambios de requerimientos durante el desarrollo, algo especialmente relevante en un proyecto académico con restricciones de tiempo.

SaludEC es una plataforma web accesible de servicios públicos de salud del Ecuador, construida con React 19 + Vite 6 + Firebase. El desarrollo real se ejecutó en \textbf{cuatro iteraciones} comprendidas entre el 28/06/2026 y el 29/06/2026. A partir de esa base funcional, el Sprint 2 (30/06/2026 -- 13/07/2026) \textbf{se completó el 13/07/2026}. Se completaron HU-11, HU-14, HU-19 y las nuevas HU-23 a HU-27, alcanzando conformidad sustancial WCAG 2.2 AA con correcciones de contraste, vídeo accesible con subtítulos VTT, eliminación de la sección Biblioteca, navegación con scroll restaurado, logo clickable y declaración de accesibilidad honesta.
...
\begin{table}[H]
\centering
\caption{Valores XP y su aplicación en SaludEC}
\small
\begin{tabular}{@{}>{\bfseries\color{violetxp}}p{3cm} p{11.5cm}@{}}
\toprule
\rowcolor{violetxp!10}
\textbf{Valor XP} & \textbf{Aplicación concreta en SaludEC} \\
\midrule
Comunicación &
...
\textbf{Criterios de aceptación:}
\begin{itemize}
  \item \texttt{document.title} se actualiza en cada cambio de ruta con formato ``Sección | SaludEC''.
  \item La página de inicio usa ``SaludEC --- Servicios públicos de salud''.
\end{itemize}
...
El Sprint 1 comprende el desarrollo completo de la plataforma SaludEC, desde la inicialización del proyecto hasta la versión con accesibilidad completa (WCAG 2.2 AA, salvo 2 criterios parciales). Se ejecutó en cuatro sub-iteraciones de trabajo intensivo.
...
\end{axis}
\end{tikzpicture}
\caption{Burndown del Sprint 1 --- SaludEC (39 puntos, 4 iteraciones)}
\end{figure}
...
PageSpeed (móvil, 4G lenta) & Inicio & 92 / 100 & Accessibility --- 03/07/2026 \\
PageSpeed (móvil, 4G lenta) & Inicio & 66 / 100 & Performance score \\
PageSpeed (móvil, 4G lenta) & Inicio & 96 / 100 & Best Practices \\
PageSpeed (escritorio) & Inicio & 92 / 100 & Accessibility --- 03/07/2026 \\
PageSpeed (escritorio) & Inicio & 80 / 100 & Performance score \\
PageSpeed (escritorio) & Inicio & 100 / 100 & SEO \\
WAVE & Inicio & 0 errores; 2 contraste; AIM 10/10 & Alertas SPA (falsos positivos) \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\rowcolor{warningyellow!15}
HU-18 & Corregir ARIA prohibido: 2 elementos detectados (Lighthouse 92) & 2 & Erick Costa & 2 \\
HU-19 & Corregir contraste: 2 errores WAVE/Lighthouse (1.4.3) & 2 & Jonathan Tipán & \textcolor{successgreen}{\cmark~Completado (13/07)} \\
HU-20 & Reducir CLS de 0.326 a $<$0.1: fuentes + skeleton shimmer & 3 & Javier Quilumba & 2 \\
HU-21 & Optimizar rendimiento móvil: imágenes w=400, JS tree-shaking & 3 & Erick Costa & 2 \\
HU-22 & Añadir cabeceras HTTP (CSP, COOP) + \texttt{llms.txt} & 2 & Jonathan Tipán & 2 \\
\midrule
\rowcolor{successgreen!10}
HU-23 & Vídeo accesible con subtítulos VTT en Nosotros (WCAG 1.2.2) & 3 & Javier Quilumba & \textcolor{successgreen}{\cmark~Completado (13/07)} \\
HU-24 & Eliminar Biblioteca Digital y limpiar referencias & 2 & Erick Costa & \textcolor{successgreen}{\cmark~Completado (13/07)} \\
HU-25 & Declaración de accesibilidad honesta (Lighthouse 92, pendientes) & 2 & Jonathan Tipán & \textcolor{successgreen}{\cmark~Completado (13/07)} \\
HU-26 & Corrección navegación: scroll al inicio, logo clickable, foco & 2 & Javier Quilumba & \textcolor{successgreen}{\cmark~Completado (13/07)} \\
HU-27 & Validación de formulario por campo (blur sin disrumpir foco) & 2 & Erick Costa & \textcolor{successgreen}{\cmark~Completado (13/07)} \\
\midrule
\rowcolor{graylight}
\multicolumn{2}{@{}l}{\textbf{Total Sprint 2}} & \textbf{54} & & \\
\bottomrule
\end{tabular}
\end{table}
...
\end{tikzpicture}
\caption{Calendario del Sprint 2 --- SaludEC}
\end{figure}

\subsection{Avance real del Sprint 2 (al 13/07/2026 --- CERRADO)}

\begin{iterbox}{Sprint 2 --- Cierre al 13/07/2026 (43+ commits acumulados)}{tealxp}
\textbf{Historias completadas:} HU-11, HU-14, HU-19, HU-23, HU-24, HU-25, HU-26, HU-27

\textbf{Commits representativos de la semana 2 (06/07--13/07):}
\begin{itemize}
  \item \texttt{e85f197} --- fix(contacto): validación en tiempo real por campo sin disrumpir foco (Erick)
  \item \texttt{b7d2d9f} --- fix(navbar): logo y eslogan SaludEC navegan al inicio al hacer clic (Jonathan)
  \item \texttt{ab6fdac} --- fix(routing): volver al inicio de la página al navegar entre rutas (Javier)
  \item \texttt{5de4984} --- fix(contraste): textos estadísticas Home con bajo contraste (Erick)
  \item \texttt{f80a43f} --- fix(contraste): textos footer y accesibilidad con bajo contraste (Jonathan)
  \item Commit nosotros: feat(nosotros): vídeo accesible MSP/OPS con subtítulos VTT (WCAG 1.2.2) (Javier)
  \item Commit biblioteca: feat(biblioteca): eliminar sección y redirigir a inicio (Erick)
  \item Commit accesibilidad: fix(accesibilidad): declaración honesta con estado real y pendientes documentados (Jonathan)
\end{itemize}

\textbf{Entregable:} Contraste WCAG 1.4.3 corregido en 3 secciones; vídeo accesible con subtítulos en Nosotros; Biblioteca eliminada; navegación con scroll restaurado; formulario de contacto con validación blur; declaración de accesibilidad actualizada y honesta.
\end{iterbox}
...
\begin{enumerate}
  \item \cmark~WCAG 2.5.7 documentado: alternativa funcional al cubo vía Navbar.
  \item \cmark~WCAG 3.2.6 documentado: enlace de ayuda en footer (Contacto) en todas las páginas.
  \item \cmark~Alt/title en imágenes de listados (miniaturas en grids de artículos/noticias).
  \item \cmark~Vídeo accesible con subtítulos VTT en Nosotros (WCAG 1.2.2).
  \item \cmark~Al menos 3 artículos por módulo de salud en Firestore.
  \item \cmark~Contraste WCAG 1.4.3 corregido: estadísticas Home, footer y declaración de accesibilidad.
  \item \cmark~Sección Biblioteca eliminada y referencias limpias en toda la aplicación.
  \item \cmark~Navegación: scroll al inicio en PUSH/REPLACE, logo clickable, validación blur sin disrupción.
  \item \cmark~Declaración de accesibilidad honesta: Lighthouse 92/100 real, 4 criterios parciales documentados, 5 correcciones pendientes listadas.
  \item \cmark~axe DevTools: 0 violaciones en todas las páginas principales al finalizar el sprint.
  \item \wmark~Lighthouse Accessibility: 92/100 (issue ARIA prohibido activo; no alcanzó objetivo de 95/100).
  \item \xmark~CLS (escritorio): 0.326 --- no corregido en este sprint; queda como deuda técnica.
\end{enumerate}
...
Total de commits (Sprint 2, al 13/07)  & $\sim$15 (acumulado: $\sim$49) \\
Líneas de código aproximadas (src/)     & $\sim$5.600 \\
Páginas implementadas                   & 10 (Biblioteca eliminada) \\
Componentes reutilizables               & 13 \\
Rutas SPA                               & 16 \\
Colecciones Firestore                   & 5 \\
Criterios WCAG 2.2 implementados        & 24 / 28 evaluados \\
Criterios WCAG 2.2 parciales            & 4 (1.2.3, 2.5.7, 3.2.6, 4.1.2) \\
Violaciones axe DevTools (producción)   & 0 \\
Lighthouse Accessibility (13/07/2026)   & 92 / 100 (issue ARIA prohibido activo) \\
PageSpeed Performance --- móvil         & 66 / 100 \\
PageSpeed Performance --- escritorio    & 80 / 100 \\
CLS (escritorio)                        & 0.326 (deuda técnica, no corregido) \\
...
\begin{enumerate}
  ...
  \item \textbf{Las iteraciones cortas aceleraron la retroalimentación:} Cuatro sub-iteraciones en Sprint 1 y quince commits en Sprint 2 permitieron detectar y corregir ocho problemas de accesibilidad (contraste 1.4.3, vídeo 1.2.2, navegación 2.4.3, foco de formulario, logo inaccesible, Biblioteca obsoleta, scroll de página, declaración inflada) antes de la entrega final.

  \item \textbf{La propiedad colectiva del código funcionó:} Los $\sim$49 commits distribuidos entre tres integrantes muestran contribución real y alternada. Ningún módulo pertenece a una sola persona; todos pueden modificar cualquier parte del proyecto.

  \item \textbf{El Sprint 2 se completó en el plazo previsto (13/07/2026):} Con 54 puntos de historia planificados y ejecutados, el Sprint 2 cerró con conformidad sustancial WCAG 2.2 AA. Las deudas técnicas pendientes (CLS, ARIA prohibido) están documentadas en la declaración de accesibilidad.
\end{enumerate}
...
\begin{enumerate}[label={[\arabic*]}]
  ...
  \item Repositorio del proyecto: \url{https://github.com/zeuspyEC/SaludEC}
